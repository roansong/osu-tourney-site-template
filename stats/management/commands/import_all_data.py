...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with open(os.path.join(settings.BASE_DIR, 'stats/data/pool_beatmaps.txt')) as f:
            pool_beatmaps = [line.strip() for line in f.readlines()]

        print(f"Beatmaps in pool: {len(pool_beatmaps)}")

        with open(os.path.join(settings.BASE_DIR, 'stats/data/multilinks.txt')) as f:
                ...
                for game_obj in games_objs:
                    ...
                    for score_obj in score_objs:
                        ...
                        try:
                            ...
                            user = models.User.objects.create(username=user_obj.get('username'), ext_id=user_id)
